    ...
    while true do 
        ...
        if pressedenter then 
            ...
            if IsControlJustPressed(1, 191) then 
                ...
                ESX.ShowNotification("~g~Bienvenue !~s~ \n~s~Voici le lien Discord du serveur : ~g~discord.gg/uk6NpCf")
                destorycam()
...
local LieuDeSpawn1 = {
	...
	Data = { currentMenu = "Lieu de spawn" },
    Events = {onSelected = function(self, _, btn, result)
            if btn.name == "Los Santos" then
                incamera = true
                createcam(false)
                self:CloseMenu(true)
                createcamfin(false)
                ...
                TeleportFadeEffect(GetPlayerPed(-1), {x = -490.52899169922, y = -681.75378417969, z = 33.251959838867})
                TriggerEvent("esx:showNotification", "Métro.\nDirection ~g~Los Santos.")
                TriggerEvent("esx:showNotification", "Poste de police.\n~g~Allez faire votre carte.")
                ESX.UI.HUD.SetDisplay(1.0)
                TriggerEvent('es:setMoneyDisplay', 1.0)
                TriggerEvent('esx_status:setDisplay', 1.0)
                DisplayRadar(true)
                TriggerEvent('ui:toggle', true)
            elseif btn.name == "Blaine County" then
                incamera = true
                createcam(false)
                self:CloseMenu(true)
                createcamfin(false)
                ...
                PlaySoundFrontend(-1, "1st_Person_Transition", "PLAYER_SWITCH_CUSTOM_SOUNDSET", 1)
                TeleportFadeEffect(GetPlayerPed(-1), {x = -408.74328613281, y = 6060.4995117188, z = 31.500106811523})
                TriggerEvent("esx:showNotification", "Taxi.\nDirection ~g~Blaine County.")
                TriggerEvent("esx:showNotification", "Poste de sheriff.\n~g~Allez faire votre carte.")
                ESX.UI.HUD.SetDisplay(1.0)
                TriggerEvent('es:setMoneyDisplay', 1.0)
                TriggerEvent('esx_status:setDisplay', 1.0)
                DisplayRadar(true)
	...
	Menu = {
        ["Lieu de spawn"] = {
            useFilter = true,
			b = {
                {name = "Los Santos", ask = "→", askX = true, Description = "Choisissez de spawn à Los Santos."},
                {name = "Blaine County", ask = "→", askX = true, Description = "Choisissez de spawn à Blaine County."}
			}
        },
    }
}
...
local creationPerso = {
	...
	Events = {onSelected = function(self, _, btn, CMenu, menuData, currentButton, currentSlt, result)
	...
	Menu = {
        ["Nouveau personnage"] = {
            useFilter = true,
			b = {
                {name = "Skin", slidemax = 564, Description = "~r~Attention ! ~s~Les skin 'ped' ne peuvent être que très légèrement modifiés."},
        ...
        ["identité"] = {
			b = {
                {name = "Prénom", ask = 'Aucun', Description = "Choisissez votre prénom."},
                {name = "Nom", ask = 'Aucun', Description = "Choisissez votre nom."},
                {name = "Date de naissance", ask = 'JJ/MM/AAAA', Description = "Choisissez votre date de naissance."},
                {name = "Taille", ask = '180', Description = "Choisissez votre taille. (en cm)"},
