...
local mincops = 4 -- minimum required cops to start mission
local enablesound = true -- enables bank alarm sound
local lastrobbed = 0 -- don't change this
local cooldown = 86400 -- amount of time to do the heist again in seconds (30min)
            ...
            if copcount >= mincops then
                if method == 1 then
                    ...
                    if item >= 1 then
                        ...
                        cb("Vous n'avez pas de charges thermiques")
                    end
                elseif method == 2 then
                    ...
                    if item >= 1 then
                        ...
                        cb("Vous n'avez pas d'outil de crochetage")
                    end
                end
            else
                cb("Il faut au minimum "..mincops.." policier en ville.")
            end
        else
            cb(math.floor((cooldown - (os.time() - lastrobbed)) / 60)..":"..math.fmod((cooldown - (os.time() - lastrobbed)), 60).." restantes avant le prochain braquage.")
        end
    else
