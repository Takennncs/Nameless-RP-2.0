...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Démarrer un ~g~GROS~w~ braquage",
        silentstart = "[~r~E~w~] Démarrer un ~g~PETIT~w~ braquage",
        usecard = "[~r~E~w~] Utiliser la carte d'indentité",
        usethermal = "[~r~E~w~] Utiliser une charge thermique",
        usehack = "[~r~E~w~] Hacker la sécurité",
        uselockpick = "[~r~E~w~] Crocheter la porte",
        usesearch = "[~r~E~w~] Chercher",
        lootcash= "[~r~E~w~] Récupérer l'argent",
        lootgold = "[~r~E~w~] Récupérer l'or",
        lootdia = "[~r~E~w~] Récupérer le diamant",
        card = "Utilisation de la carte",
        thermal = "Activation de la charge",
        burning = "Fondage de la serrure",
        lockpick = "Crochetage de la serrure",
        using = "Accès au pannel",
        used = "Processus complété",
        stage = "complété",
        search = "Rechere",
        hacking = "Hacking",
        melted = "Serrure fondu",
        hacked = "Hack complété",
        unlocked = "Porte dévérouiller",
        nothing = "Vous n'avez rien trouvé",
        found = "Vous avez trouvé la carte d'identité",
        time = "Restant: "
    }
}
UTK.searchinfo = {
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                ESX.ShowNotification('Vous n\'avez pas de ~g~PC Portable')
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            ESX.ShowNotification('Vous n\'avez pas la ~g~carte d\'identité')
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                ESX.ShowNotification('Vous n\'avez pas de ~g~charge thermique')
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    ESX.ShowNotification('Vous n\'avez pas ~g~d\'outil de crochetage')
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Plant()
    ...
    exports["a_loadingbar"]:StartDelayedFunction('Activation de la charge', 4500, function() end) 
    Citizen.Wait(1500)
    ...
    Citizen.Wait(4000)
    exports["a_loadingbar"]:StartDelayedFunction('Fondage de la serrure', 12000, function() end) 
    DeleteObject(bag)
    ...
    Citizen.Wait(9000)
    ESX.ShowNotification('La serrure est ~g~fondu')
...
function UTK:Lockpick()
    ...
    exports["a_loadingbar"]:StartDelayedFunction('Crochetage de la serrure', 6000, function() end) 
    Citizen.Wait(6500)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    ESX.ShowNotification('Ouvrez ~g~My Computer~s~ et accédez à ~g~HackConnect.exe')
...
function UTK:IdCard()
    ...
    exports["a_loadingbar"]:StartDelayedFunction('Utilisation de la carte', 2000, function() end) 
    Citizen.Wait(1500)
    ...
    elseif self.currentid == 2 then
        ...
        UTK.disableinput = false
        exports["a_loadingbar"]:StartDelayedFunction('Accès au pannel', 4000, function() end)
        local count = 4
        repeat
            Citizen.Wait(1000)
            count = count - 1
        until count == 0
        ESX.ShowNotification('Processus ~g~complété')
        SpawnObj()
...
function Search(location)
    ...
    exports["a_loadingbar"]:StartDelayedFunction('Recherche en cours', 15000, function() end)
    Citizen.Wait(15000)
    if UTK.searchinfo.random ~= 1 then
        location.status = true
        ESX.ShowNotification('Vous n\'avez ~g~rien ~s~trouvé')
        UTK.searchinfo.random = math.random(1, UTK.cur - 1)
        UTK.cur = UTK.cur - 1
    else
        UTK.searchinfo.found = true
        ESX.ShowNotification('Vous avez trouvé la ~g~carte d\'indentité')
...
function Process(ms, text)
    exports["a_loadingbar"]:StartDelayedFunction(text, ms, function() end)
    Citizen.Wait(ms)
end
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    ESX.ShowNotification('Chercher ~g~l\'adresse IP~s~...')
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    ESX.ShowNotification('Chercher ~g~le mot de passe~s~...')
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    ESX.ShowNotification('Exécutez ~g~BruteForce.exe')
                ...
                elseif Hacking and program == 86 then
                    ...
                    if UTK.hackmethod == 1 then
                        --TriggerServerEvent("utk_oh:toggleDoor", UTK.hack1.type, vector3(UTK.hack1.x, UTK.hack1.y, UTK.hack1.z), false, UTK.hack1.h)
                        TriggerServerEvent("utk_oh:policeDoor", 3, false)
                        ESX.ShowNotification('Hack ~g~réussi')
                        UsingComputer = false
                        UTK.disableinput = false
                        UTK.hackfinish = true
                        Ipfinished = false
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        ESX.ShowNotification('Vous avez ~g~90 secondes ~s~avant l\'arrivé du ~g~gaz toxique')
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            ESX.ShowNotification('Vous avez volé '..text)
        end)
    else
        ESX.ShowNotification('Vous avez ~g~90 secondes ~s~avant l\'arrivé du ~g~gaz toxique')
        UTK.starttimer = true
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        ESX.ShowNotification('Les portes se ferment dans ~g~60 secondes~s~ !')
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                ESX.ShowNotification('Les portes se ~g~referment~s~ !')
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    TriggerServerEvent("call:makeCall", "police", {x=UTK.loudstart.x,y=UTK.loudstart.y,z=UTK.loudstart.z}, "Braquage de la pacific bank !")
    TriggerServerEvent("call:makeCall", "sheriff", {x=UTK.loudstart.x,y=UTK.loudstart.y,z=UTK.loudstart.z}, "Braquage de la pacific bank !")
    if player.job.name == "police" or player.job.name == "sheriff" then
        if toggle == 1 then
            if not DoesBlipExist(UTK.alarmblip) then
	            ...
	            SetBlipScale(UTK.alarmblip, 0.7)
