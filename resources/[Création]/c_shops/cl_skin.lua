...
function spawner_menu()

    local elems = {
        {label = ('Los Santos'),     value = 'lsspawner'},
		{label = ('Blaine County'),     value = 'bcspawner'},
    }

    ESX.UI.Menu.CloseAll()

    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'settings',
    {
        css = 'head',
        title    = 'Choisissez un endroit ci-dessous',
        align = 'top-left',
        elements = elems
    ...
    function(data, menu)
		if data.current.value == 'lsspawner' then 
			--destorycam()
			menu.close()
			ESX.LoadingPrompt("Vous avez décidé d'apparaître à Los Santos", 20)
			Citizen.Wait(5000)
			RemoveLoadingPrompt()
			TeleportFadeEffect(GetPlayerPed(-1), {x = -491.0,  y = -728.8,  z = 23.92-0.90})
			TriggerEvent("esx:showNotification", "Aéroport.\nDirection ~g~Los Santos.")
			TriggerEvent("esx:showNotification", "Commissariat.\n~g~Allez faire votre carte.")
		elseif data.current.value == 'bcspawner' then 
			--destorycam()
			menu.close()
			ESX.LoadingPrompt("Vous avez décidé d'apparaître à Blaine County", 20)
			Citizen.Wait(5000)
			RemoveLoadingPrompt()
			TeleportFadeEffect(GetPlayerPed(-1), {x = 1758.69, y = 3297.85, z = 41.15-0.90, a = 2.48})
...
function OpenMenu(submitCb, cancelCb, restrict)
	...
	TriggerServerEvent('instance:create', 'createskin', {owner = playerPed})
	...
	TriggerEvent('skinchanger:getData', function(components, maxVals)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'skin', {
			css  = 'head',
			title = "Personalisation de votre personnage",
			align = 'top-left',
			elements = elements
		}, function(data, menu)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'confirm_escape', {
				css  = 'head',
				title = "Êtes vous sur de vouloir annuler ?",
				align = 'top-left',
				elements = {
					{label = "Non",  value = 'no'},
					{label = "Oui", value = 'yes'}
...
function createcam(default)
    ...
    if (not DoesCamExist(cam)) then
        if default then
            cam = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', 403.04, -998.6, -99.0, 0.0, 0.0, 359.92, 85.0, false, 0)
        end
        SetCamActive(cam, true)
