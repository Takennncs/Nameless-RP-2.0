...
AddEventHandler('esx_accessories:pay', function()
	...
	xPlayer.removeMoney(Config.Pricemask)
	TriggerClientEvent('esx:showNotification', _source, _U('you_paid') .. '$' .. Config.Pricemask)
...
ESX.RegisterServerCallback('esx_accessories:checkMoney', function(source, cb)
	...
	if xPlayer.get('money') >= Config.Pricemask then
		cb(true)
	else
		cb(false)
	end
