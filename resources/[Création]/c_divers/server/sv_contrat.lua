	...
	if result[1] ~= nil then
		MySQL.Async.execute('UPDATE owned_vehicles SET owner = @target WHERE owner = @owner AND plate = @plate', {
			...
			if rowsChanged ~= 0 then
				...
				Wait(22000)
				TriggerClientEvent('esx:showNotification', _source, 'Vous avez vendu un véhicule avec comme plaque : ', plate)
				TriggerClientEvent('esx:showNotification', _target, 'Vous avez acheté le véhicule aevc comme plaque : ', plate)
