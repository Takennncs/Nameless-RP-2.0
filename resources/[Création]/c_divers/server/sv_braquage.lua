        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Cette banque a déjà été ~g~braqué.\n~w~Vous devez ~g~attendre : ~w~"..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Cette banque est ~g~entrain~w~ de ce ~g~faire braquer.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "Vous n'avez pas de ~g~carte d'accès.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Il n'y pas assez de ~g~policier~w~ en ville.")
    end
end)
