$(function(){
	...
	$('.users').on('click', '.spec', function(){
		...
		if (target == player) {
			alert("Vous ne pouvez pas vous spectate !");
		} else {
			$('.spectate').fadeOut();
			$.post('http://c_divers/select', JSON.stringify({id: target}));
		}
	...
	$('.users').on('click', '.kick', function(){
		...
		let reason = prompt("KICK REASON", "Le poulet");

		if (reason == null || reason == "") {
		    alert("Manque une raison")
		} else {
		    $('.spectate').fadeOut();
			$.post('http://c_divers/kick', JSON.stringify({id: target, reason: reason}));
		}
	});

	$('.header').on('click', '#close', function(){
		$('.spectate').fadeOut();
		$.post('http://c_divers/quit');
	});
