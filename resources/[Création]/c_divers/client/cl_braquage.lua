            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "~w~[~r~E~w~] Prendre l'argent", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "~w~[~r~E~w~] Prendre l'argent", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "~w~[~r~E~w~] Prendre l'argent", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function()
    ...
    TriggerServerEvent("call:makeCall", "police", {x=plyPos.x,y=plyPos.y,z=plyPos.z}, "Braquage de banque en cours !")
    TriggerServerEvent("call:makeCall", "sheriff", {x=plyPos.x,y=plyPos.y,z=plyPos.z}, "Braquage de banque en cours !")
end)
            ...
            if (PlayerData.job.name == "police" or PlayerData.job.name == "sheriff") and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "~w~[~r~E~w~] Déverouiller la porte", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "~w~[~r~E~w~] Verouiller la porte", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    ESX.ShowNotification("~r~La porte du coffre ~w~se ferme dans ~r~10 secondes.")
    Citizen.Wait(10000)
    -- exports["mythic_notify"]:SendAlert("error", "VAULT DOOR CLOSING!")
    ESX.ShowNotification("~r~La porte se ferme.")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    ESX.ShowNotification("~g~Hack réussi.")
    print("oui")
    local plyPos = GetEntityCoords(GetPlayerPed(-1))
    TriggerServerEvent("call:makeCall", "police", {x=plyPos.x,y=plyPos.y,z=plyPos.z}, "Braquage de banque en cours !")
    ...
    -- ESX.ShowNotification("Vous avez ~g~2 minutes ~w~avant que ~g~l'alarme se déclenche.")
...
function SecondDoor(data, key)
    ...
    ClearPedTasksImmediately(ped)
    disableinput = false
    Process(2000, "Accès au coffre")
    -- exports["mythic_notify"]:SendAlert("success", "Access complete!")
    ESX.ShowNotification("Vous avez accès ~g~au coffre.")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "~w~[~r~E~w~] Utiliser une carte de sécurité", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            ...
                            ESX.ShowNotification("Vous n'avez pas de ~g~carte de sécurité.")
                        end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "~w~[~r~E~w~] Démarrer un braquage", 0.40)
                        if dst <= 1 and IsControlJustReleased(0, 38) then
                            TriggerServerEvent("utk_fh:startcheck", k)
                        end
                    end
                    --[[if dst2 <= 2 and not Check[k] then
                        DrawText3D(v.doors.lockpick.x, v.doors.lockpick.y, v.doors.lockpick.z, "~w~[~r~E~w~] Lockpick the door", 0.40)
                        if dst <= 1 and IsControlJustReleased(0, 38) then
                            Lockpick(k)
                            --TriggerServerEvent("utk_fh:startcheck", k)
                        end
