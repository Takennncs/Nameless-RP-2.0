...
AddEventHandler('esx_jb_radars:ShowRadarBlip', function()
	for k, v in pairs(Config.Radars) do
		...
		SetBlipScale(RadarBlip[k], 0.6)
		SetBlipAsShortRange(RadarBlip[k], true)

		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Radars")
							...
							if fine ~= 0 then
								SetTimeout(1500, function()
								  local maxSpeed = speedCam.maxSpeed
									if PlayerData.job.name == 'police' and PlayerData.job.name ==  'ambulance' and PlayerData.job.name ==  'sheriff' then
										ESX.ShowNotification("~r~Vous venez de passer un radar à toute vitesse.")
									...
									--elseif PlayerData.job ~= nil and PlayerData.job.name ==  'ambulance' then
										...
										ESX.ShowNotification("~b~Plaque flaché :~r~ "..plate.."\n~b~Vitesse :~r~ "..kmhSpeed.."~b~\nAmende :~r~ -"..fine.."$")
