...
--     while true do
--       Wait(0)
--       if IsControlJustReleased(0, Config.OpenKey) and (GetGameTimer() - GUI.Time) > 1000 and GetLastInputMethod(2) then
--         openmenuvehicle()
--         GUI.Time = GetGameTimer()
--       end
--     end
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
  ESX.TriggerServerCallback("esx_trunk:getInventoryV",function(inventory)
      ...
      TriggerEvent("c_inventaire:openTrunkInventory", data, inventory.blackMoney, inventory.cash, inventory.items, inventory.weapons)
    end,
    plate)
end
