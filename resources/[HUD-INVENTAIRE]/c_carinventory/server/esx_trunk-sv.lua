      ...
      if targetItem.limit == -1 or ((targetItem.count + count) <= targetItem.limit) then
        TriggerEvent(
          ...
          function(store)
            ...
            TriggerClientEvent("c_inventaire:refreshTrunkInventory", _source, data, blackMoney, items, weapons)
          end
        )
      else
        ...
        TriggerEvent(
          ...
          function(store)
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item) * count)) > max then
                TriggerClientEvent("esx:showNotification", _source, "~r~Espace insuffisant.")
            else
