...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        $(".nearbyPlayerButton").click(function () {
            ...
            $.post("http://c_inventaire/GiveItem", JSON.stringify({
                player: player,
...
function closeInventory() {
    $.post("http://c_inventaire/NUIFocusOff", JSON.stringify({}));
}
...
$(document).ready(function () {
    ...
    $('#use').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://c_inventaire/UseItem", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#give').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://c_inventaire/GetNearPlayers", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://c_inventaire/DropItem", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                ...
                $.post("http://c_inventaire/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://c_inventaire/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://c_inventaire/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "shop" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://c_inventaire/TakeFromShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://c_inventaire/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://c_inventaire/PutIntoProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://c_inventaire/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
