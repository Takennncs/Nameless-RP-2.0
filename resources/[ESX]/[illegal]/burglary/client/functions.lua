...
LockHouse = function(id, data)
    ...
    while true do
        if IsEntityPlayingAnim(PlayerPedId(), 'missheistfbisetup1', 'hassle_intro_loop_f', 3) then
            ...
            ESX.ShowNotification('Vous avez annulé la fermeture.')
            break
        end
        drawTxt(0.97, 0.6, 1.0, 1.0, 0.5, "Fermeture... ~b~" .. math.floor((timer / 1000) / Config.LockTime * 100) .. "~s~%", 255, 255, 255, 255)
        Wait(0)
    end
end
    ...
    Citizen.CreateThread(function()
        ...
        while cachedData["InsideHouse"] do
            ...
            if dstcheck <= 5.5 then
                ...
                ESX.Game.Utils.DrawText3D(type["Sortir"], text, 0.6)
                DrawMarker(6, type["Sortir"]-vector3(0.0,0.0,0.975), 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 1.0, 1.0, 1.0, 55, 100, 200, 155, 0, false, false, 0, false, false, false, false)
            end
...
CreateBlip = function(eventData)
    ...
    SetBlipScale(cachedData[eventData["houseId"]], 0.7)
	SetBlipColour(cachedData[eventData["houseId"]], 1)
	BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(Config.Coords[eventData["houseId"]]["Info"])
    EndTextCommandSetBlipName("Cambriolage")
    PulseBlip(cachedData[eventData["houseId"]])
end

CreateBlipp = function(eventData)
    cachedData[eventData["houseId"]] = AddBlipForCoord(Config.Coords[eventData["houseId"]]["Pos"])
    SetBlipSprite(cachedData[eventData["houseId"]], 134)
    SetBlipScale(cachedData[eventData["houseId"]], 0.7)
    SetBlipDisplay(cachedData[eventData["houseId"]], 8)
	SetBlipColour(cachedData[eventData["houseId"]], 1)
	BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(Config.Coords[eventData["houseId"]]["Info"])
    EndTextCommandSetBlipName("Cambriolage")
...
LootPlace = function(data, id, LootablesId, LootablesData)
    ...
    while GetGameTimer() - cachedData["startedSearching"] < Config.SearchTime do
        ...
        if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
            ESX.ShowNotification("Vous avez arrêter de fouiller.")
    ...
    ESX.TriggerServerCallback("burglary:lootItem", function(loot)
        if loot then
            -- ESX.ShowNotification("Vous avez récupérer " ..loot)
        end
    end)
...
EnterCameraMode = function(id, data)
    Citizen.CreateThread(function()
        ...
        DrawBusySpinner("Connection au caméra de sécurité")
        ...
        while cachedData["cameraMode"] do
            ...
            DrawButtons({
                ...
                },
                {
                    ...
                    ["label"] = "Caméra suivante"
                },
                {
                    ["button"] = "~INPUT_CELLPHONE_LEFT~",
                    ["label"] = "Caméra précédente"
                }
            })
            ...
            elseif IsControlJustReleased(0, 177) then
                DrawBusySpinner("Déconnection des caméras.")
                DoScreenFadeOut(1500)
                Wait(2500)
                ExitInstance()
