...
function OnPlayerDeath()
	...
	ESX.LoadingPrompt("Vous êtes dans le coma.", 20)
...
function StartDistressSignal()
	Citizen.CreateThread(function()
		...
		while timer > 0 and IsDead do
			...
			ESX.ShowNotification("~r~Coma.\n~s~Voulez-vous contacter une ambulance ?")
			ESX.ShowNotification("Accepter : ~g~G")

			if IsControlPressed(1, Keys['G']) then
				--SendDistressSignal()
				--ESX.ShowHelpNotification("Message envoyé")
				ESX.LoadingPrompt("Message envoyé", 20)
				Citizen.Wait(6500)
				ESX.LoadingPrompt("Vous êtes dans le coma.", 20)
				--RemoveLoadingPrompt()
				local plyPos = GetEntityCoords(GetPlayerPed(-1), true)
				TriggerServerEvent("call:makeCall", "mort", {x=plyPos.x,y=plyPos.y,z=plyPos.z})
...
AddEventHandler('esx_ambulancejob:notif', function()
	...
	ESX.ShowNotification("~g~Réanimation effectué.\n+ ~g~150$ ~s~Entreprise.")
end)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				...
				ESX.ShowNotification("Appuyez sur ~g~E~s~ pour utiliser l'unité ~r~X~s~ (500$)")
