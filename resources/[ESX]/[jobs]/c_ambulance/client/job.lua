...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		...
		elements = {
			...
			{ label = 'Donne une facture <span style="color:;"> >',   value = 'billing' },
			{label = '<span style="color:rgb(2, 117, 214);">EMS Disponible',     value = 'ems_ouvert'},
			{label = '<span style="color:rgb(2, 117, 214);">EMS non Disponible',     value = 'ems_ferme'}
		}
	}, function(data, menu)
		...
		elseif data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = 'Sortir une chaise roulante', value = 'wheelchair'},
					{label = 'Supprimer une chaise roulante', value = 'wheelchairsuppr'}
				}
			}, function(data, menu)
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									--Citizen.Wait(10000)
									exports["a_loadingbar"]:StartDelayedFunction("Vous soignez...", 10000, function()
		...
		for k in pairs(posnorthcar) do 
			...
			if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
				if dist <= 10.0 then 
					...
					if dist <= 1.5 then 
						ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~ouvrir le garage")
						if IsControlJustPressed(1, 51) then 
