...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	ESX.ShowAdvancedNotification("Central", "~b~Appel d'urgence: 912", "~b~Identité: ~s~Piéton\n~b~Localisation: \n~s~"..GetStreetNameFromHashKey(GetStreetNameAtCoord(x, y, z)).."\n~b~Infos:\n~s~Personne dans le ~r~Coma~s~.", 'CHAR_CALL911', 7) -- E X
     ...
     while true do
		...
		if AppelEnAttente then
			...
			elseif IsControlJustPressed(1, 73) and AppelEnAttente then
				ESX.ShowNotification("~r~Vous avez refuser l'appel.")
		...
		if IsControlJustPressed(1, 38) and AppelDejaPris == true then
			ESX.ShowNotification("~r~L'appel à déja été pris")
		end
     end
end)
