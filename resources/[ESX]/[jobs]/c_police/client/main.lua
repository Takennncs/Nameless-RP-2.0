	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			...
			if job == 'bullet_wear' then
				-- TriggerServerEvent('addbulletpolice')
...
function spawnCarHeli(car)
	...
	local vehicle = CreateVehicle(car, -1096.157, -832.34, 37.70-0.50, 304.9, true, false)
...
function spawnCar(car)
	...
	local vehicle = CreateVehicle(car, -1070.015, -852.288, 4.86-0.85, 215.0, true, false)
	SetVehicleNumberPlateText(vehicle, "LSPD")
	SetEntityAsMissionEntity(vehicle, true, true)
	local plate = GetVehicleNumberPlateText(vehicle)
	TriggerServerEvent('esx_vehiclelock:givekey', 'no', plate) 
...
local posdeletecarpolice = {
	{x = -1078.7227783203, y = -845.33734130859, z = 4.8840656280518},
	{x = 449.232, y = -981.297,  z = 43.691}
}
		...
		elseif data.current.value == 'gestion_appels' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gestion_appels', {
				...
				elseif action == 'renfort' then
					...
					TriggerServerEvent("call:makeCall", "renforts", {x=plyPos.x,y=plyPos.y,z=plyPos.z})
					Citizen.Wait(2000)
				end
...
function AlertePolice()
	...
	TriggerServerEvent('police:posalertes', playerX, playerY, playerZ)
end

RegisterNetEvent('police:alertenotif')
AddEventHandler('police:alertenotif', function(x, y, z)	
	if PlayerData.job.name == 'police' then
