...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			ShowHelpNotif("Appuyez sur ~INPUT_TALK~ pour ~r~annuler le plein." .. extraEssence .. extraString)
		else
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								ESX.ShowHelpNotification('Appuyez sur ~INPUT_TALK~ pour ~g~faire le plein')
						...
						elseif not canFuel then
							...
							ESX.ShowNotification('~g~Vous avez terminé de mettre de l\'essence.')
						end
					end
