	...
	'default', GetCurrentResourceName(), 'mykey',
	{
        ...
        ESX.UI.Menu.CloseAll()
  			ESX.UI.Menu.Open(
				...
				align = 'top-left',
				elements = {
						{label = 'Donner votre paire de Clé', value = 'donnerkey'}, -- Donné les clés
	...
	while not HasAnimDictLoaded(dict) do
		...
		if gotkey then
			...
			if locked == 1 or locked == 0 then -- if unlocked
				...
				--startAnim("anim@mp_player_intmenu@key_fob@", "fob_click")
				TriggerEvent('InteractSound_CL:PlayOnOne', 'lock', 1.0)
				ESX.ShowNotification("Véhicule ~r~vérouillé.")
				--ESX.ShowNotificaton("Véhicule ~g~Déverouillé.")
				--TriggerEvent('esx:showAdvancedNotification', source, '~b~Paire de Clé !', 'Vous avez ~r~Fermé ~w~votre Vehicule', 'CHAR_MP_FM_CONTACT', 23)
			...
			elseif locked == 2 then -- if locked
				...
				--TriggerEvent('esx:showAdvancedNotification', source, '~b~Paire de Clé !', 'Vous avez ~g~Ouvert ~w~votre Vehicule', 'CHAR_MP_FM_CONTACT', 23)
				SetVehicleLights(vehicle, 2)
                Wait(200)
                SetVehicleLights(vehicle, 0)
                --StartVehicleHorn(vehicle, 100, 1, false)
			...
			--TriggerEvent('esx:showAdvancedNotification', source, '~b~Paire de Clé !', '~r~Vous n\'avez pas les clés de ce véhicule', 'CHAR_MP_FM_CONTACT', 23)
			ESX.ShowColoredNotification("Aucun véhicule trouvé.", 8)
		end
...
RegisterKeyMapping('+lockcar', 'Utiliser ses clés', 'keyboard', 'U')
...
Citizen.CreateThread(function()
		...
		AddTextComponentString('Clé Voiture')
...
Citizen.CreateThread(function()
		...
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('Clé Moto')
		EndTextCommandSetBlipName(blip)
end)
...
Citizen.CreateThread(function()
		...
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('Clé Publique')
		EndTextCommandSetBlipName(blip)
end)

-- Create Blips
Citizen.CreateThread(function()
		local blip = AddBlipForCoord(Config.Zones.place4.Pos.x, Config.Zones.place4.Pos.y, Config.Zones.place4.Pos.z)
		SetBlipSprite (blip, 134)
		SetBlipDisplay(blip, 4)
		SetBlipScale  (blip, 0.7)
		SetBlipColour (blip, 3)
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('cle Publique')
		EndTextCommandSetBlipName(blip)
end)
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0, 38) then
        ...
        if CurrentAction == 'cle' then
          OpencleMenu(CurrentActionData.zone)
        end
