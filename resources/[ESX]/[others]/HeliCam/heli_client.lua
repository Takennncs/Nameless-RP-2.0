...
local supervolito_hasg = GetHashKey("supervolito")
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if IsPlayerInPolmav() or IsPlayerInSupervolito() then
            ...
            if IsHeliHighEnough(heli) then
                ...
                if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
                    ...
                    if GetPedInVehicleSeat(heli, 1) == lPed or
                        ...
                        AddTextComponentString("~r~Vous ne pouvez pas descendre en rappel depuis cette place.")
...
function IsPlayerInSupervolito()
    local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
    return IsVehicleModel(vehicle, supervolito_hash)
end
