	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				-- --, Source, "SYSTEM", {255, 0, 0}, "You do not have permission to do this")
			end
		end)
	end)
end)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if not available then
							-- --, Source, 'SYSTEM', {255, 0, 0}, "Your group can not use this command.")
						else
							-- --, Source, 'SYSTEM', {255, 0, 0}, "Permission denied.")
						end
					end
				end)
			end)
		...
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			...
			if t == "group" then
				if(GetPlayerName(USER) == nil)then
					-- --, source, 'SYSTEM', {255, 0, 0}, "Player not found")
				else
					TriggerEvent("es:getAllGroups", function(groups)
						if(groups[GROUP])then
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								...
								-- --, -1, "CONSOLE", {0, 0, 0}, "Group of ^2^*" .. GetPlayerName(tonumber(USER)) .. "^r^0 has been set to ^2^*" .. GROUP)
							end)
						else
							-- --, source, 'SYSTEM', {255, 0, 0}, "Group not found")
						end
					end)
				end
			elseif t == "level" then
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP, function(response, success)
							if(true)then
								--, -1, "CONSOLE", {0, 0, 0}, "Permission level of ^2" .. GetPlayerName(tonumber(USER)) .. "^0 has been set to ^2 " .. tostring(GROUP))
							end
						end)	
					else
						--, source, 'SYSTEM', {255, 0, 0}, "Invalid integer entered")
					end
				end
			elseif t == "money" then
				if(GetPlayerName(USER) == nil)then
			...
			elseif t == "bank" then
				...
				--, source, 'SYSTEM', {255, 0, 0}, "superadmin required to do this")
			end
		end)
	end)	
end)
	...
	if commandName == 'setadmin' then
		...
		TriggerEvent("es:setPlayerData", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
			...
			if(true)then
				...
				--, -1, "CONSOLE", {0, 0, 0}, "Permission level of ^2" .. GetPlayerName(tonumber(args[1])) .. "^0 has been set to ^2 " .. args[2])
			end
		end)
			...
			if(groups[args[2]])then
				TriggerEvent("es:setPlayerData", tonumber(args[1]), "group", args[2], function(response, success)
					...
					if(true)then
						...
						--, -1, "CONSOLE", {0, 0, 0}, "Group of ^2^*" .. GetPlayerName(tonumber(args[1])) .. "^r^0 has been set to ^2^*" .. args[2])
					end
				end)
			else
	...
	elseif commandName == 'setmoney' then
			...
			TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
				if(user)then
					...
					RconPrint("Money set")
					--, tonumber(args[1]), "CONSOLE", {0, 0, 0}, "Your money has been set to: ^2^*$" .. tonumber(args[2]))
				end
			end)
...
TriggerEvent('es:addCommand', 'admin', function(source, args, user)
	--, source, "SYSTEM", {255, 0, 0}, "Level: ^*^2 " .. tostring(user.get('permission_level')))
	--, source, "SYSTEM", {255, 0, 0}, "Group: ^*^2 " .. user.getGroup())
end, {help = "Shows what admin level you are and what group you're in"})

-- Default commands
--TriggerEvent('es:addCommand', 'report', function(source, args, user)
--	--, source, "REPORT", {255, 0, 0}, " (^2" .. GetPlayerName(source) .." | "..source.."^0) " .. table.concat(args, " "))

--	TriggerEvent("es:getPlayers", function(pl)
--		for k,v in pairs(pl) do
--			TriggerEvent("es:getPlayerFromId", k, function(user)
--				if(user.getPermissions() > 0 and k ~= source)then
--					--, k, "REPORT", {255, 0, 0}, " (^2" .. GetPlayerName(source) .." | "..source.."^0) " .. table.concat(args, " "))
--				end
--			end)
--		end
--	end)
...
TriggerEvent('es:addGroupCommand', 'noclip', "admin", function(source, args, user)
	...
	--, source, "SYSTEM", {255, 0, 0}, "Insufficienct permissions!")
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				--, -1, "SYSTEM", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been kicked(^2" .. reason .. "^0)")
				DropPlayer(player, reason)
			end)
		else
			--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
	else
		--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
	end
...
TriggerEvent('es:addGroupCommand', 'annonce', "admin", function(source, args, user)
	--, -1, "ANNONCE ", {255, 0, 0}, "" .. table.concat(args, " "))
end, function(source, args, user)
	--, source, "SYSTEM", {255, 0, 0}, "Insufficienct permissions!")
end, {help = "Envoie un message a tout le monde (chat)", params = {{name = "annonce", help = "Le message de l'annonce"}}})
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				--, player, "SYSTEM", {255, 0, 0}, "You have been " .. state .. " by ^2" .. GetPlayerName(source))
				--, source, "SYSTEM", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been " .. state)
			end)
		else
			--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				--, player, "SYSTEM", {255, 0, 0}, "You have brought by ^2" .. GetPlayerName(source))
				--, source, "SYSTEM", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been brought")
			end)
		else
			--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				--, player, "SYSTEM", {255, 0, 0}, "You have slapped by ^2" .. GetPlayerName(source))
				--, source, "SYSTEM", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been slapped")
			end)
		else
			--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
		...
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					--, player, "SYSTEM", {255, 0, 0}, "You have been teleported to by ^2" .. GetPlayerName(source))
					--, source, "SYSTEM", {255, 0, 0}, "Teleported to player ^2" .. GetPlayerName(player) .. "")
				end
			end)
		else
			--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
...
TriggerEvent('es:addCommand', 'die', function(source, args, user)
	...
	--, source, "", {0,0,0}, "^1^*You killed yourself.")
end, {help = "Suicide"})
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				--, player, "SYSTEM", {255, 0, 0}, "You have been killed by ^2" .. GetPlayerName(source))
				--, source, "SYSTEM", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been killed.")
			end)
		else
			--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				--, source, "SYSTEM", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been crashed.")
			end)
		else
			--, source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
...
TriggerEvent('es:addGroupCommand', 'pos', "owner", function(source, args, user)
	--, source, "SYSTEM", {255, 0, 0}, "This command was removed")
end, function(source, args, user)
	--, source, "SYSTEM", {255, 0, 0}, "This command was removed")
